...
ENV = os.getenv('ENV', 'test')  # Default to test environment for safety
env_file = f'.env.{ENV}'
...
BASECAMP_MESSAGE_BOARD_ID = os.getenv("BASECAMP_MESSAGE_BOARD_ID", "8279891628")  # Default to prod for backward compatibility
...
def post_to_basecamp(md_file_path, subject="Daily OS Report"):
    ...
    # 3) Build the endpoint URL (using the format that worked in production)
    message_endpoint = f"https://3.basecampapi.com/{BASECAMP_ACCOUNT_ID}/buckets/{BASECAMP_PROJECT_ID}/message_boards/{BASECAMP_MESSAGE_BOARD_ID}/messages.json"
    ...
    # 4) Send request
    r = requests.post(message_endpoint, headers=headers, json=post_data)
...
def get_web_signals(repo_name):
    """
    Gather web signals for a repository by searching for mentions in tech media,
    ...
    try:
        # Search HN/Reddit discussions
        hn_query = f'site:news.ycombinator.com "{repo_name}"'
        hn_results = search_web(query=hn_query)
        if hn_results:
            signals["hn_discussions"] = hn_results[:3]  # Top 3 discussions

        # Search tech blogs and media
        blog_query = f'site:medium.com OR site:dev.to OR site:techcrunch.com "{repo_name}"'
        blog_results = search_web(query=blog_query)
        if blog_results:
            signals["tech_blogs"] = blog_results[:3]

        # Search for company/enterprise mentions
        company_query = f'"{repo_name}" (enterprise OR production OR "case study" OR "success story")'
        company_results = search_web(query=company_query)
        if company_results:
            signals["company_mentions"] = company_results[:3]

        # Search for maintainer info
        maintainer = repo_name.split('/')[0]  # Get the owner part
        maintainer_query = f'"{maintainer}" (linkedin OR twitter OR personal blog OR speaker OR conference)'
        maintainer_results = search_web(query=maintainer_query)
        if maintainer_results:
            signals["maintainer_info"] = maintainer_results[:2]

        logging.info(f"Web signals gathered for {repo_name}")

    except Exception as e:
        logging.error(f"Error gathering web signals for {repo_name}: {e}")
